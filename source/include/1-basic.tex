...
\begin{faq}{什么是TDS}
  TDS 全称 TeX Directory Structure，意为 TeX 目录结构，即 TeX 发行版的文件组织结构。大部分 TeX 发行版都将自身的文件组织成相近的路径结构，也就是 TDS。TDS 也称为 TEXMF 树，这是 TeX 与 METAFONT 的合称。很多系统的 TDS 结构都以 texmf 或者类似的词作为 TEXMF 树的根目录名，如在 TeX Live 中，安装目录下的 texmf-dist、texmf-var 等就是两个不同的 TEXMF 树，如图 \ref{texmf-dir}。
  \begingroup
    \makeatletter
    \def\@captype{figure}
    \makeatother
    \dirtree {%
      ...
      .3 texlive/\DTcomment{TeX Live 发行版自身的文档}.
      ...
      .5 report.cls.
      .5 latex.ltx.
      .4 beamer/\DTcomment{beamer 宏包相关文件}.
