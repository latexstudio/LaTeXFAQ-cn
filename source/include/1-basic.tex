...
Overleaf 编译也自动启用 |--shell-escape|，因此\pkg{minted}、\pkg{tikz-externalize} 等都可以使用。|--shell-escape| 本来是有安全隐忧的，但是每个 Overleaf 文件项目都是一个单独的 Docker 盒子 (container)，因此若是有谁想玩什么把戏，也不会波及其它项目。
